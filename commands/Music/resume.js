module.exports.run = async ({ client, message, args, prefix }) => {
    ...
    if (!player.paused) return message.reply(`${client.uncheckEmoji} Looks like the music is already resumed.`)
    .then(msg => { setTimeout(() => msg.delete(), 6969) })
 
    try {

        player.pause(false)
        return message.reply(`:arrow_forward: Resumed`)
...
module.exports.help = {
    name: 'resume',
    aliases: [],
    description: 'Resumes the paused music in a VC.',
    usage: 'resume'
}
